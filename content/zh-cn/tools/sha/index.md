---
title: "SHA 哈希生成工具"
description: "在线 SHA 哈希生成工具。快速生成 SHA-1, SHA-256, SHA-384, 和 SHA-512 哈希值。"
date: 2026-03-20
layout: "page"
category: "security"
keywords: ["SHA", "哈希", "摘要"]
tool_related: ["md5", "password"]
---

安全哈希算法（SHA）是由美国国家安全局（NSA）设计的一系列密码哈希函数。它们广泛用于安全应用和数据完整性验证。

{{< tool id="sha" >}}

### 示例与限制
输入 `abc` 并选择 SHA-256（小写），结果为 `ba7816bf8f01cfea414140de5dae2223b00361a396177a9cb410ff61f20015ad`。支持 SHA-1、SHA-256、SHA-384 和 SHA-512；文本在浏览器本地计算，不会上传。

### 如何使用 SHA 生成器
1.  在输入框中输入需要计算哈希的文本或字符串。
2.  从下拉菜单中选择所需的 SHA 算法（SHA-1, SHA-256, SHA-384, 或 SHA-512）。
3.  选择输出格式（小写或大写）。
4.  哈希值会在您输入时自动生成，或者您可以点击“生成”按钮。
5.  复制生成的哈希值供您使用。

### 可用算法
-   **SHA-1**：产生 160 位（40 个字符）的哈希值。虽然在密码学中已不再被认为是安全的，但仍用于非安全任务，如 Git 校验和。
-   **SHA-256**：SHA-2 家族的一部分，产生 256 位（64 个字符）的哈希值。它是目前使用最广泛的哈希函数之一（例如在比特币中）。
-   **SHA-384**：SHA-2 家族的一员，产生 384 位（96 个字符）的哈希值。
-   **SHA-512**：SHA-2 家族的一员，产生 512 位（128 个字符）的哈希值。

> **提示**：哈希用于计算摘要，不是可解密的加密。一般摘要可选择 SHA-256；需要抗碰撞能力时不要使用 SHA-1。密码存储应使用 Argon2id、scrypt 等专用方案，而不是直接计算 SHA 哈希。
