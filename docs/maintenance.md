...
`.github/workflows/site-monitor.yml` 每周一 06:00 UTC 运行，也支持手动触发。它不重新部署，只访问 `https://blog.codeglimpse.top`，检查：
...
监控失败时先查看失败端点，再根据最近部署提交判断是代码、主题、资源、DNS 还是暂时性网络问题。监控不会自动修改代码或回滚。
